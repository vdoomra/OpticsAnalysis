...
void generating_csv_files(string infile, const int cut, double nFiles){
  ...
  for(int ihole = 0; ihole < nholes; ihole++){
    ...
    csvFile << "tg_th" << "," << "tg_ph" << "," << "tg_p" << "," << "tg_vz" << "," << "sieve_r" << "," << "central_sieve_r" << "," << "gem_r" << "," << "r_prime" << "," << "gem_ph" << "," << "phi_prime" << "," << "phi_local" << "," << "rate" << endl; 
    ...
    for(int j=0; j<nevents; j++){
      ...
      if( IsInside(ellipse_rphi, gem_r, gem_ph) && IsInside(ellipse_rrprime, r_prime, gem_ph) && IsInside(ellipse_phi_phiprime, phi_prime, gem_ph) ){  
        ...
        csvFile << tg_th << "," << tg_ph << "," << tg_p << "," << tg_vz << "," << sieve_r << "," << central_sieve_r[index_hole] << "," << gem_r << "," << r_prime << "," << gem_ph << "," << phi_prime << "," << phi_local << "," << rate << endl;
        csvFile_merged << tg_th << "\t" << tg_ph << "\t" << tg_p << "\t" << tg_vz << "\t" << sieve_r << "\t" << central_sieve_r[index_hole] << "\t" << gem_r << "\t" << r_prime << "\t" << gem_ph << "\t" << phi_prime << "\t" << phi_local << "\t" << rate << endl;
