...
def PolynomialRegression( X, y, degree, variable):  
  ...
  hist, bin_edges, _ = plt.hist(y_res_test, bins=100, density=True)
  ...
  plt.hist(y_res_test, bins=100, density=True, alpha=0.7, label='Histogram')
...
if __name__=='__main__':
  ...
  all_target = [["opticsUS","opticsUM","opticsMS","opticsMD","opticsDS"], ["opticsDS","opticsMS","opticsUS"]]
  ...
  for i in range(2):
    ...
    x_values=np.array([-512.45, -480, -450, -420, -387.55]) # Location of the C Foil
    means = []
    ...
    for itr in range(5):
      ...
      for a_target in all_target[i]:
        ...
        d[a_target]['vz_pred'] = (-d[a_target].central_sieve_r/np.tan(d[a_target].theta_pred) + 105.)/10.
        d[a_target]['residual_correction'] = (coefs[0]*d[a_target].vz_pred*d[a_target].vz_pred + coefs[1]*d[a_target].vz_pred + coefs[2])/1000
        d[a_target]['theta_pred'] = d[a_target]['theta_pred'] + d[a_target]['residual_correction']
        d[a_target]['theta_residuals'] = d[a_target]['tg_th'] - d[a_target]['theta_pred']
        d[a_target]['vz_residuals'] = d[a_target]['tg_vz'] - d[a_target]['vz_pred']*10
      ...
      hist, bin_edges, _ = plt.hist(all_df_new.vz_residuals, bins=100, density=True)
      bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
      initial_guess = [np.mean(all_df_new.vz_residuals), np.std(all_df_new.vz_residuals)]
      params, covariance = curve_fit(gaussian, bin_centers, hist, p0=initial_guess)
      mu, sigma = params

      x = np.linspace(min(bin_centers), max(bin_centers), 100)
      fitted_curve = gaussian(x, mu, sigma)
      plt.hist(all_df_new.vz_residuals, bins=100, density=True, alpha=0.7, label='Histogram')
      plt.plot(x, fitted_curve, 'r-', label='Fitted Gaussian')
      plt.xlim(-1000,1000)
      plt.xlabel('Residuals')
      plt.legend()
