...
def main():
    ...
    sourceDir = "/work/halla/moller12gev/vdoomra/new_optics/remoll"
   
    activeDetectors = [28, 32, 33, 34, 35, 270, 9000, 9001]

    energyscan = [2.2, 4.4, 6.6, 8.8]

    make_tarfile(sourceDir)
   
    for energy in energyscan:

       config = "sieve_C12_opticsMD_" + str(energy)
       ...
       nrEv   = 10000 #100000
       nrStart= 0
       nrStop = 500 #(nrStop -nrStart) 3000
...
def createMacFile(srcDir, outDirFull,nrEv,jobNr, detectorList,energy):
    ...
    f.write("/remoll/addfield "+srcDir+"/map_directory/subcoil_2_3_3mm_full.txt\n")
    f.write("/remoll/evgen/set elasticC12\n")
    f.write("/remoll/rasx 0 mm\n")
    f.write("/remoll/rasy 0 mm\n")
    f.write("/remoll/evgen/thmin 0.2 deg\n")
    f.write("/remoll/evgen/thmax 1.4 deg\n")
...
def createSBATCHfile(sourceDir,outDirFull,jobName,jobNr):
    ...
    f.write("#SBATCH --mem-per-cpu=3500\n")
...
def make_tarfile(sourceDir):
    ...
    tar.add(sourceDir+"/macros/target/Optics3.mac", arcname="macros/target/Optics3.mac")
    ...
    tar.add(sourceDir+"/geometry/upstream/blocker.gdml" ,arcname="geometry/upstream/blocker.gdml")
