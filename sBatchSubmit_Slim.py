...
def main():
    ...
    for energy in energyscan:
       ...
       config = "slim_sieve_C12_opticsDS_" + str(energy)
       outDir = "/volatile/halla/moller12gev/vdoomra/"+config
       srcDir = "/volatile/halla/moller12gev/zdemirog/sieve_C12_opticsDS_"+str(energy)
...
def createShellScript(srcDir,outDirFull):
    ...
    f=open(fname,"w")
    f.write("#!/bin/sh\n")
    f.write("LIST=`ls -lrt "+ srcDir + "/*/o_remoll.root "+"| "+"awk '{print $9}'`\n")
    f.write("NUM=0\n")
    f.write("for file in $LIST\n")
    f.write("do\n")
    f.write("if (( $NUM == $1 ))\n")
    f.write("then\n")
    f.write("cp /work/halla/moller12gev/vdoomra/new_optics/SlimGeneral.C .\n")
    f.write("root -l -b << EOF\n")
    f.write(".X SlimGeneral.C(\"$file\")\n")
    f.write("EOF\n")
    f.write("fi\n")
    f.write("NUM=$(( $NUM + 1 ))\n")
    f.write("done\n")
...
def createSBATCHfile(outDirFull,jobName,jobNr):
    ...
    f.write("#SBATCH --mem-per-cpu=2500\n")
